...
import app.androwara.data.model.friends.Friend
import app.androwara.data.model.friends.FriendList
import app.androwara.data.model.friends.FriendStatus
...
import app.androwara.data.model.user.UserFriendState
import app.androwara.util.isNotEmpty
import app.androwara.util.net.*
    ...
    suspend fun getVideoPageDetail(session: Session, videoId: String): Response<VideoDetail> =
        withContext(Dispatchers.IO) {
            try {
                ...
                var likeLink = ""
                if (isNotEmpty(likeFlag) && isNotEmpty(isLike)) {
                    likeLink = URLDecoder.decode(
                ...
                var isFollow = false
                var followLink = ""
                if (isNotEmpty(followFlag)) {
                    isFollow = (followFlag?.attr("href")?.startsWith("/flag/unflag/") == true)
                    followLink = followFlag?.attr("href").let {
                        it?.substring(it.indexOf("/follow/") + 8).toString()
                    }
                }
